...
//    private int[] defaultLineColor = {Color.RED, Color.YELLOW, Color.WHITE};
    private int[] defaultLineColor = {Color.TRANSPARENT, Color.YELLOW, Color.WHITE};
//    private int hintColor = Color.RED;
    private int hintColor = Color.TRANSPARENT;
    /**
     * xy轴文字颜色和大小
     */
     ...
     * value：一组数据
     */
    private Map<Integer, List<Units>> datas = new HashMap<>();
    ...
    private int[] findSuitEdgeInVisual() {
        ...
        if (offset == 0) {// 不可滑动或当前位于最左边
            ...
            if (datas.get(0).size() > maxOfVisible) {
                ...
                List<Units> i = datas.get(0);
                while (low <= high) {
                    int mid = (low + high) >>> 1;
                    Units midVal = i.get(mid);
    ...
    private void drawLines(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            for (int j = 0; j < datas.size(); j++) {
                Units current = datas.get(j).get(i);
                ...
                if (lineType == SEGMENT) {
                    ...
                    // 到这里肯定不是起始点，所以可以减1
                    Units previous = datas.get(j).get(i - 1);
    ...
    private void drawClickHint(Canvas canvas) {
        Units cur = datas.get(clickIndexs[1]).get(clickIndexs[0]);
    ...
    private void feedInternal(Map<Integer, List<Units>> entry, List<Paint> entryPaints, boolean needAnim) {
        cancelAllAnims();
     ...
     * @param datas
     */
    private void calcMaxUnit(Map<Integer, List<Units>> datas) {
        // 先“扁平”
        List<Units> allUnits = new ArrayList<>();
        for (List<Units> line : datas.values()) {
            allUnits.addAll(line);
        }
        // 再拷贝，防止引用问题
        List<Units> bakUnits = new ArrayList<>();
        for (int i = 0; i < allUnits.size(); i++) {
            bakUnits.add(allUnits.get(i).clone());
        }
        // 最后排序，得到最大值
        Collections.sort(bakUnits);
        Units maxUnit = bakUnits.get(bakUnits.size() - 1);
        Units minUnit = bakUnits.get(0);
    ...
    private long calcTotalCost() {
        ...
        return oneLineCost + oneLineCost / percentOfStartNextLineAnim * (datas.size() - 1) + Units.DURATION + 16;
    }
    ...
    private void showWithAnims() {
        ...
        for (int i = suitEdge[0]; i <= suitEdge[1]; i++) {
            for (List<Units> item : datas.values()) {
    ...
    private void startLineAnim(final int startIndex, final int endIndex) {
        final List<Units> line = datas.get(curAnimLine);
...
//    }
     ...
     * @param line
     */
    public void feedWithAnim(List<Units> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<Units>> entry = new HashMap<>();
        entry.put(0, line);
     ...
     * @param line
     */
    public void feed(List<Units> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<Units>> entry = new HashMap<>();
        entry.put(0, line);
        handler.post(new Runnable() {
    ...
    public static class LineBuilder {
        ...
        private Map<Integer, List<Units>> datas;
         ...
         * @return
         */
        public LineBuilder add(List<Units> data, int... color) {
